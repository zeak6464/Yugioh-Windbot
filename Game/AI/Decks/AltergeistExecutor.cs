...
namespace WindBot.Game.AI.Decks
{
    ...
    public class AltergeistExecutor : DefaultExecutor
    {
        ...
        List<int> cards_improper = new List<int>
        {
            ...
            CardId.Manifestation, CardId.SecretVillage, CardId.ImperialOrder,   CardId.HarpiesFeatherDuster, CardId.GR_WC,
        ...
        List<int> normal_counter = new List<int>
        {
            ...
            74762582, 75286651, 4810828,  44665365, 21123811, CardId.CrystalWingSynchroDragon,
            82044279, 82044280, 79606837, 10443957, 1621413,  CardId.Protocol,
            90809975, 8165596,  9753964,  53347303, 88307361, CardId.GamecieltheSeaTurtleKaiju,
            5818294,  2948263,  6150044,  26268488, 51447164, CardId.JizukirutheStarDestroyingKaiju,
            97268402
        };
        ...
        {
            // negate
            AddExecutor(ExecutorType.Activate, CardId.ChickenGame, ChickenGame);
            ...
            AddExecutor(ExecutorType.Activate, CardId.HarpiesFeatherDuster, Feather_activate);
        ...
        public bool Storm_activate()
        {
            ...
            if (Duel.Phase == DuelPhase.End 
                ...
                || (Util.IsChainTarget(Card) 
                    || (Util.GetLastChainCard() != null && Util.GetLastChainCard().Controller == 1 && Util.GetLastChainCard().IsCode(CardId.HarpiesFeatherDuster))))
            {
        ...
        public bool Spoofing_eff()
        {
            ...
            foreach(ClientCard card in Bot.GetSpells())
            {
                if ( (Util.ChainContainsCard(CardId.HarpiesFeatherDuster) || Util.IsChainTarget(card)) 
            ...
            if (!go)
            {
                foreach (ClientCard card in Bot.GetMonsters())
                {
                    if (card.IsFaceup() && (Util.IsChainTarget(card) || Util.ChainContainsCard(CardId.DarkHole) || (!Protocol_activing() && card.IsDisabled()))
                        && Duel.LastChainPlayer != 0 && isAltergeist(card))
                    {
