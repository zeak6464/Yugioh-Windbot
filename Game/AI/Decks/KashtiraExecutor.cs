    ...
    class KashtiraExecutor : DefaultExecutor
    {
        ...
        public override IList<ClientCard> OnSelectCard(IList<ClientCard> cards, int min, int max, long hint, bool cancelable)
        {
            ...
            if (hint == HintMsg.XyzMaterial && cards.Any(card => card != null && card.Location == CardLocation.Removed) && min == 1 && max == 1)
            {
                ...
                if (m_pre_cards != null && m_pre_cards.Count > 0) res.AddRange(m_pre_cards);
            ...
            if (hint == HintMsg.Release && cards.Any(card => card != null && card.Location == CardLocation.MonsterZone))
            {
                ...
                if (opt_1)
                {
                    ...
                    if(tRelease != null && tRelease.Count > 0) nRelease.AddRange(tRelease);
            ...
            if (activate_pre_PressuredPlanetWraitsoth)
            {
                ...
                if (res != null && res.Count > 0) return Util.CheckSelectCount(res, cards, min, max);
                return null;
        ...
        {
            if (cardsList == null || cardsId == null) return new List<ClientCard>();
            List<ClientCard> res = new List<ClientCard>();
        ...
        private bool IPSummon()
        {
            ...
            if (cards == null || cards.Count < 2 && !link_mode) return false;
            ...
            if (pre_cards != null && pre_cards.Count >= 2) { AI.SelectMaterials(pre_cards); return true; }
            cards.Sort(CardContainer.CompareCardAttack);
            AI.SelectMaterials(cards);
            return true;
        }
        private bool IPEffect()
        {
            ...
            if (pre_m == null || pre_m.Count <= 0) return false;
            int link_count = 0;
        ...
        private bool KashtiraAriseHeartSummon()
        {
            ...
            {
                List<ClientCard> materials = Bot.GetMonsters().Where(card => !card.IsExtraCard() && card.IsFaceup() && Card.HasSetcode(0x189)).ToList();
        ...
        private bool KashtiraRiseheartEffect_2()
        {
            if (Card.Location != CardLocation.Hand)
            {
                if (CheckRemainInDeck(CardId.KashtiraBigBang) > 0 && Bot.GetMonsters().GetMatchingCardsCount(card => card != null && card.HasType(CardType.Xyz)
                        && card.HasSetcode(0x189) && card.IsFaceup() && card.Overlays.Count > 0) > 0)
                {
