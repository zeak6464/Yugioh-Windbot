...
namespace WindBot.Game.AI.Decks
{
    ...
    class WitchcraftExecutor : DefaultExecutor
    {
        public class CardId
        {
            ...
            public const int HarpiesFeatherDuster = 40640003;
        }
        ...
        public override void OnChaining(int player, ClientCard card)
        {
            ...
            if (!MagicianRightHand_used && card.IsSpell() && card.Controller == 1)
            {
                    ...
                    && Bot.HasInSpellZone(CardId.MagicianRightHand, true))
                {
                    ...
                    Logger.DebugWriteLine("MagicianRightHand negate: " + cardName);
            ...
            if (!MagiciansLeftHand_used && card.IsTrap() && card.Controller == 1)
            {
                    ...
                    && Bot.HasInSpellZone(CardId.MagiciansLeftHand, true))
                {
                    string cardName = card.Name;
                    if (cardName == null) cardName = "???";
                    Logger.DebugWriteLine("MagiciansLeftHand negate: " + cardName);
        ...
        public override IList<ClientCard> OnSelectCard(IList<ClientCard> cards, int min, int max, long hint, bool cancelable)
        {
            ...
            if (hint == HintMsg.AddToHand)
            {
                ...
                if (flag)
                {
                    string cardName = cards[cards.Count - 1].Name;
                    if (cardName == null) cardName = "???";
                    ...
                    for (int i = 1; i <= max; ++i)
                    {
                        ...
                        string cardName2 = cards[cards.Count - i].Name;
                        if (cardName2 == null) cardName2 = "???";
                        Logger.DebugWriteLine("** Select " + cardName2);
                    }
                    return selected;
                }
            }
            ...
            if (hint == HintMsg.Confirm)
            {
                ...
                foreach (ClientCard card in cards)
                {
                    string cardName = card.Name;
                    if (cardName == null) cardName = "???";
                    Logger.DebugWriteLine(cardName);
                }
                ...
                {
                    selected.Add(cards[cards.Count - i]);
                    string cardName = cards[cards.Count - i].Name;
                    if (cardName == null) cardName = "???";
                    Logger.DebugWriteLine("** Select " + cardName);
                }
                return selected;
            }
        ...
        public override CardPosition OnSelectPosition(int cardId, IList<CardPosition> positions)
        {
            ...
            if (!Enemy.HasInMonstersZone(CardId.BlueEyesChaosMAXDragon)
        ...
        /// <param name="need_tuner">whether need tuner</param>
        /// <returns></returns>
        public bool CheckLinkMaterialsMatch(int LinkCount, int MaterialCount, List<ClientCard> list, bool need_tuner = false)
        {
            ...
            // tuner check
            if (need_tuner && list.GetFirstMatchingCard(card => card.IsTuner()) == null) return false;
            return true;
        }
        ...
        public ClientCard CheckProblematicCards(bool canBeTarget = false, bool OnlyDanger = false)
        {
            ...
            int highest_self = Util.GetBestAttack(Bot);
        ...
        public bool HaineActivate()
        {
            ...
            if (targetcard != null)
            {
                string cardName = targetcard.Name;
                if (cardName == null) cardName = "???";
                Logger.DebugWriteLine("*** Haine target: "+ cardName);
                SelectDiscardSpell();
        ...
        public int HolidayCheck(ClientCard except_card = null){
            ...
            if (CheckProblematicCards() == null)
            {
                ...
                foreach (int cardid in check_list)
                {
                    if (!UseSSEffect.Contains(cardid) && Bot.Graveyard.GetFirstMatchingCard(card => card.Id == cardid && card != except_card) != null && CheckDiscardableSpellCount() > 0)
                    {
        ...
        public bool InfiniteImpermanenceActivate()
        {
            ...
            if (Card.Location == CardLocation.SpellZone)
            {
                ...
                if ((this_seq * that_seq >= 0 && this_seq + that_seq == 4)
                    ...
                    || (LastChainCard != null && LastChainCard.Controller == 1 && LastChainCard.IsCode(CardId.HarpiesFeatherDuster)))
                {
        ...
        public bool MasterpieceActivate()
        {
            ...
            // ss effect
            {
                ...
                int discardable_hands = CheckDiscardableSpellCount();
                List<int> will_discard_list = new List<int> { CardId.Haine, CardId.MadameVerre, CardId.Schmietta, CardId.Pittore, CardId.Genni, CardId.Potterie };
        ...
        public bool ChickenGame()
        {
            ...
            if (Bot.LifePoints <= 1000)
                return false;
            if (Bot.LifePoints - 1000 <= Enemy.LifePoints && ActivateDescription == Util.GetStringId(CardId.ChickenGame, 0))
            {
                return true;
            }
            if (Bot.LifePoints - 1000 > Enemy.LifePoints && ActivateDescription == Util.GetStringId(CardId.ChickenGame, 1))
            {
                return true;
            }
            return false;
