...
namespace WindBot.Game.AI.Decks
{
    ...
    public class GrenMajuThunderBoarderExecutor : DefaultExecutor
    {
        ...
        {
            if (ActivateDescription == Util.GetStringId(CardId.BorreloadDragon, 0) && (Duel.Phase==DuelPhase.BattleStart||Duel.Phase==DuelPhase.End))
            {
        ...
        {
            if (attacker.IsCode(CardId.EaterOfMillions) && (Bot.HasInMonstersZone(CardId.InspectBoarder) && eater_eff) && !attacker.IsDisabled())
            {
                attacker.RealPower = 9999;
                return true;
            }
            if (attacker.IsCode(CardId.EaterOfMillions) && !Bot.HasInMonstersZone(CardId.InspectBoarder) && !attacker.IsDisabled())
            {
                attacker.RealPower = 9999;
                return true;
            }            
