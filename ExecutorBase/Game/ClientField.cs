    ...
    public class ClientField
    {
        ...
        public int GetLinkedZones()
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                {
                    zones |= MonsterZone[i].GetLinkedZones();
                }
            }
            return zones;
        }
        ...
        public IEnumerable<ClientCard> GetCards(CardLocation location, CardType type = 0)
        {
            IEnumerable<ClientCard> cards = null;
            ...
            switch (location)
            {
                ...
                case CardLocation.MonsterZone:
                    cards = MonsterZone.Where(card => card != null);
                    break;
                case CardLocation.SpellZone:
                    cards = SpellZone.Where(card => card != null);
                    break;
            ...
            if (type != 0)
            {
                if (cards != null)
                {
                    cards = cards.Where(card => card.HasType(type));
                }
            }

            return cards ?? Enumerable.Empty<ClientCard>();
        }
    }
}
