...
namespace WindBot.Game
{
    ...
    public class GameAI
    {
        ...
        {
            string cardName = card != null ? card.Name : "null";
            AILogger.DebugWriteLine(string.Format("Considering activation of {0} (desc: {1})", cardName, desc));

            // Always activate these types of effects
            if (card != null)
            {
                // Spell/Trap activation
                if (card.Location == CardLocation.SpellZone || card.Location == CardLocation.Hand)
                {
                    if (card.HasType(CardType.Spell) || card.HasType(CardType.Trap))
                    {
                        AILogger.DebugWriteLine(string.Format("Activating Spell/Trap card {0}", card.Name));
                        return true;
                    }
                }
                ...
                if (card.Location == CardLocation.MonsterZone || card.Location == CardLocation.Hand)
                {
                    ...
                    {
                        // Check if it's a quick effect during opponent's turn
                        if (Duel.Player == 1 && card.HasType(CardType.QuickPlay))
                        {
                            AILogger.DebugWriteLine(string.Format("Activating Quick Effect of {0} during opponent's turn", card.Name));
                            return true;
                        }

                        // During our turn, activate most monster effects
                        if (Duel.Player == 0)
                        {
                            AILogger.DebugWriteLine(string.Format("Activating Monster Effect of {0} during our turn", card.Name));
                            return true;
                        }
                    }
                }
            ...
            // Check executors for specific card handling
            foreach (CardExecutor exec in Executor.Executors)
            {
                if (ShouldExecute(exec, card, ExecutorType.Activate, desc))
                {
                    string name = card != null ? card.Name : "null";
                    AILogger.DebugWriteLine(string.Format("Executor decided to activate {0}", name));
                    return true;
                }
            }

            // Default to activating most effects
            string defaultName = card != null ? card.Name : "null";
            AILogger.DebugWriteLine(string.Format("Default activation for {0}", defaultName));
            return true;
        }
        ...
        public BattlePhaseAction Attack(ClientCard attacker, ClientCard defender)
        {
            ...
            if (defender != null)
            {
                ...
                attacker.IsLastAttacker = false;
                _dialogs.SendAttack(attacker.Name, cardName);
                SelectCard(defender);
            }
            else
            {
                attacker.IsLastAttacker = true;
